...
// ─── O(1) — Konstan ───────────────────────────────────
function ambilPertama(arr) { return arr[0]; }           // 0(1)
function tambahkanItem(arr, item) { arr.push(item)}     // 0(1)
...
function binarySearch(arr, target){
    ...
    while (kiri <= kanan){
        ...
        if (arr[tengah] === target) {
            console.log(` Ditemukan di indeks ${tengah} serelang ${langkah} langkah`);
            return tengah;
        }
...
function fibRekursif(n) {
    if (n <= 1) return n;
    return fibRekursif(n-1) + fibRekursif(n-2);  //dua panggil rekursif
}
