...
class Stack {
    ...
    push(data){ //buat antrian node secara LIFO kala
    ...
    peek(){ //hanya melihat node paling atas
...
function evaluasiPostfix(ekspresi) {
    const stack = new Stack();
    const tokens = ekspresi.split(' ');  //["3","4","+","2","*"]
    for (const token of tokens) {
        if (!isNaN(token)) {
            stack.push(Number(token)); //masuk stack jika ketemu operator maka else
        } else {
